...
list_비례이름 <- purrr::map(paste0("data/비례대표/", list), function(x){paste0(list.files(file.path(x)))}) %>% purrr::reduce(c) %>% 
  str_extract("(?<=_).*") %>% str_remove(".xlsx")
list_비례 <- purrr::map(paste0("data/비례대표/", list), function(x){paste0(x, "/", list.files(file.path(x)))}) %>% purrr::reduce(c)

read <- function(list, name) {
  df <- readxl::read_excel(list, skip = 3)
  colnames(df)[-c(1:4)] <- df[1, -c(1:4)]
  df <- df[-1, ]
  df <- df %>% janitor::clean_names(ascii = FALSE) %>% 
...
df_비례대표 %>% group_by(정당) %>% summarise(득표 = sum(득표)) %>% arrange(desc(득표)) %>% View()

## 지역구 ----
list <- list.files(file.path("data/지역구"))
list_지역구이름 <- purrr::map(paste0("data/지역구/", list), function(x){paste0(list.files(file.path(x)))}) %>% purrr::reduce(c) %>% 
  str_extract("(?<=_).*") %>% str_remove(".xlsx")
list_지역구 <- purrr::map(paste0("data/지역구/", list), function(x){paste0(x, "/", list.files(file.path(x)))}) %>% purrr::reduce(c)
...
list_재보궐이름 <- purrr::map(paste0("data/재보궐/", list), function(x){paste0(list.files(file.path(x)))}) %>% purrr::reduce(c) %>% 
  str_extract("(?<=_).*") %>% str_remove(".xlsx")
list_재보궐 <- purrr::map(paste0("data/재보궐/", list), function(x){paste0(x, "/", list.files(file.path(x)))}) %>% purrr::reduce(c)
